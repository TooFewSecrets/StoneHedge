/proc/accuracy_check(zone, mob/living/user, mob/living/target, associated_skill, datum/intent/used_intent, obj/item/I)
	...
	if(used_intent)
		...
		if(used_intent.blade_class == BCLASS_PICK && !(target.cmode)) // Daggers are used on downed people to finish them off, not standing mid-fight. Downed strikes always hit target anyway.
